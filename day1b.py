...
for line in input.splitlines():
	print(line)
	line = line.replace("one","o1ne").replace("two","t2wo").replace("three","th3ree").replace("four","f4our").replace("five","f5ive").replace("six","s6ix").replace("seven","s7even").replace("eight","e8ight").replace("nine","n9ine")
	x = re.findall("\d",line)
	num = "%s%s" % (x[0], x[len(x)-1])
	print(">"+num)
	count = count + int(num)
